...
G_scream0 = bytearray()  #空数组
G_scream1 = bytearray()  #空数组
...
def Writet_Photo_Path4():
    ...
    if write_path4 == 0:  #确保上次执行写入完毕
        ...
        for i in range(0, 36):  #依次转换36张图片
            im1 = Image.open(Path_use + str(i) + str(write_path4))
            if im1.width >= (im1.height * 2):  #图片长宽比例超过2:1
                im2 = im1.resize((int(80 * im1.width / im1.height), 80))
                Img_m = int(im2.width / 2)
                box = (Img_m - 80, 0, Img_m + 80, 80)  #定义需要裁剪的空间
...
def Read_M_u16(add):
    ...
    #等待收回信息
    while (1):
        recv = SER_Read()  #.decode("gbk") or utf-8 #获取串口数据
        if (recv == 0):
            return 0
        elif (len(recv) != 0):
...
def Write_Flash_Photo_fast(Page_add, Photo_name):  #往Flash里面写入Bin格式的照片
    ...
    try:  #尝试打开bin文件
        bin_file = open(filepath, 'rb')  #以只读方式打开
    except:  #出现异常
...
def Write_LCD_Photo_fast1(x_star, y_star, x_size, y_size, Photo_name):
    ...
    for j in range(0, Fsize // 256):  #每次写入一个Page
        ...
        hex_use.append(0)
    if Fsize % 256 != 0:  #还存在没写完的数据
        data_w = bin_file.read(Fsize % 256)  #将剩下的数据读完
        for i in range(Fsize % 256, 256):
...
def Screen_Date_get():  #创建专门的函数来获取屏幕图像和处理转换数据
    global G_scream0_OK, G_scream1_OK, G_scream0, G_scream1, size_USE_X1, size_USE_Y1, im1
    print("截图线程创建成功")
    ...
    if size_mode == 0:  #横向充满
        ...
        size_USE_Y1 = 80
    while 1:
        if G_scream0_OK == 0 or G_scream1_OK == 0:
            u_time1 = time.time()
            hex_16RGB = []  #bytearray()
            ...
            if G_scream0_OK == 0:
                G_scream0 = Screen_Date_Process(hex_16RGB)
                G_scream0_OK = 1
            elif G_scream1_OK == 0:
                G_scream1 = Screen_Date_Process(hex_16RGB)
                G_scream1_OK = 1
            u_time1 = time.time() - u_time1
            print("\r截屏频率" + str(int(1 / u_time1)).rjust(3, " "), end="hz")
        time.sleep(0.001)


def show_PC_Screen():  #显示照片
    global State_change, Screen_Error, Device_State
    global G_scream0_OK, G_scream1_OK, G_scream0, G_scream1, size_USE_X1, size_USE_Y1
    if State_change == 1:
        State_change = 0
        Screen_Error = 0
    ...
    if State_change == 0:
        ...
        if G_scream0_OK == 1 or G_scream1_OK == 1:
            #print("传输画面ing")
            u_time = time.time()
            if G_scream0_OK == 1:
...
def Get_MSN_Device():  #尝试获取MSN设备
    global Device_State, ADC_det, ser, State_change, State_machine, My_MSN_Device, My_MSN_Data, Screen_Error, Label1, LCD_Change_now, Text1, port_list, hex_code
    ...
    if len(port_list) == 0:
        ...
        for i in range(len(port_list)):
            port = port_list[i]
            print("\r串口进行监听:" + port.device.ljust(16), end="")
            try:  # 尝试打开串口
                if os.name == 'nt':  # sys.platform == 'win32':
                    ser = serial.Serial(port.name, 19200, timeout=2)  # 初始化串口连接,初始使用
                elif os.name == 'posix':
                    ser = serial.Serial(port.device, 19200, timeout=2)  # 初始化串口连接,初始使用
                else:
                    print(f'不支持的操作系统：{os.name}')
                    continue  # 执行下一次循环
                print("FIND")
            except:  # 出现异常
                print('无法打开,请检查是否被其他程序占用或权限不足')  # 显示MSN设备数量
                time.sleep(0.1)
                continue  # 执行下一次循环
                    ...
                    if ord(recv[n]) == 0:  #当前字节为0时进行解析
                        if (recv[n + 1] == 'M') and (recv[n + 2] == 'S') and (recv[n + 3] == 'N'):  #确保为MSN设备
                            if (recv[n + 4] >= '0') and (recv[n + 4] <= '9') and (recv[n + 5] >= '0') and (
                                ...
                                My_MSN_Device.append(
                                    MSN_Device(port.name, (ord(recv[4]) - 48) * 10 + (ord(recv[5]) - 48)))  #对MSN设备进行登记
                                ...
                                if (ord(recv[0]) == 0) and (recv[1] == 'M') and (recv[2] == 'S') and (
                                    ...
                                    print(
                                        'MSN设备' + str(len(My_MSN_Device)) + '——' + port.name + '连接完成')  #显示MSN设备数量
                                else:
                                    print(
                                        'MSN设备' + str(len(My_MSN_Device)) + '无法连接,请检查连接是否正常')  #显示MSN设备数量
                                break  #退出当前for循环
...
if __name__ == '__main__':
    ScreenUIThread = threading.Thread(target=UI_Page)
    ScreenShot = threading.Thread(target=Screen_Date_get)
    ScreenUIThread.start()
    while 1:
        if ScreenUIThread.is_alive() :  #检测到用户界面被关闭
            if ScreenShot.is_alive():  #已检测到设备
